...
export const addBook = formData => async dispatch => {
    try {
        const newBookId = await axios.post(
            '/api/book',
            formData,
            AXIOS_REQUEST_CONFIG,
        );
        ...
        const newBook = await axios.get(`/api/book/${newBookId.data}`);
...
export const deleteBook = id => async dispatch => {
    const deletionSuccessful = await axios.delete(`/api/book/${id}`);
...
export const deleteTag = (bookId, tag) => async dispatch => {
    ...
    try {
        await axios.delete(`/api/book_tag/${bookId}`, {
            headers: headers,
            data: formData,
        });

        const updatedBook = await axios.get(`/api/book/${bookId}`);
...
export const updateBook = (bookId, editFormData) => async dispatch => {
    ...
    try {
        ...
        const updatedBook = await axios.get(`/api/book/${updatedBookId.data}`);
